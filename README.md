...
- Convert to standardized risks:
  ...
  \[
  R^{\mathrm{std}}_k(t) = 1 - e^{-H^{\mathrm{std}}_k(t)}
  \]

- Form the age-standardized KCOR curve:

  \[
  \mathrm{KCOR}_{\mathrm{pool}}(t) = \frac{R^{\mathrm{std}}_{\mathrm{dose}}(t)}{R^{\mathrm{std}}_{\mathrm{ref}}(t)}
  \]
...
### ⚠️ Limitations
There are 10 limitations of the method that users should be aware of. These are typically limitations that would apply anytime you are analyzing retrospective data. 
...
10. **Data quality issues:** If data is missing or unreliable, KCOR might give misleading results. For example, the Czech data is missing birthdates for over 1M unvaccinated people. This can affect age comparisons. However, this does not invalidated vaccinated to vaccinated comparisons, e.g., Dose 3 group vs. Dose 2 group.
