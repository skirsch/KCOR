...
4. KCOR COMPUTATION:
   ...
   - Step 1: Apply slope correction to mortality rates: MR_adj = MR × exp(-slope × (t - t0))
   - Step 2: Calculate adjusted deaths: D_adj = MR_adj × PT
   - Step 3: Calculate CMR from adjusted cumulative deaths: CMR = cumD_adj / cumPT
...
def build_kcor_rows(df, sheet_name):
    ...
    Assumptions:
      ...
      - CMR = cumD_adj / cumPT where cumD_adj comes from slope-adjusted mortality rates
...
def process_workbook(src_path: str, out_path: str):
    ...
    for sh in sheets_to_process:
        ...
        # Calculate adjusted deaths from adjusted mortality rates
        ...
        # Calculate CMR from adjusted cumulative deaths
        ...
        df["CMR_actual"] = np.where(df["cumPT"] > 0, df["cumD_unadj"]/df["cumPT"], np.nan)
