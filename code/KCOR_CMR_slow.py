...
print(f"Starting KCOR_CMR.py script...")
...
for enroll_date_str in enrollment_dates:
    ...
    # Compute population base: count of people in each (born, sex, dose_group) - aggregate across DCCI
    print(f"  Computing population base...")
    pop_base = a_copy.groupby(['YearOfBirth', 'Sex', 'dose_group'])['DCCI'].count().reset_index(name='pop')
    print(f"    Total population across all dose groups: {pop_base['pop'].sum()}")
    
    # Compute deaths per (WeekOfDeath, born, sex, dose_group) - aggregate across DCCI
    print(f"  Computing deaths per WeekOfDeath/born/sex/dose_group...")
    deaths = a_copy[a_copy['DateOfDeath'].notnull() & a_copy['WeekOfDeath'].notna()].groupby(['WeekOfDeath', 'YearOfBirth', 'Sex', 'dose_group'])['DCCI'].count().reset_index(name='dead')
    ...
    # Build MultiIndex for all (WeekOfDeath, born, sex, dose) - same as original
    ...
    print(f"  MultiIndex size: {len(index):,} combinations")
    print(f"  Weeks: {len(all_weeks)}, Cohorts: {len(cohorts)}, Sexes: {len(sexes)}, Doses: {len(dose_groups)}")
    ...
    for d in dose_groups:
        ...
        # deaths for this group (aggregate across DCCI values)
        deaths_d = deaths[deaths['dose_group'] == d].groupby(['WeekOfDeath', 'YearOfBirth', 'Sex'])['dead'].sum().reset_index().set_index(['WeekOfDeath', 'YearOfBirth', 'Sex'])['dead']
        # pop for this group (aggregate across DCCI values)
        pop_d = pop_base[pop_base['dose_group'] == d].groupby(['YearOfBirth', 'Sex'])['pop'].sum().reset_index().set_index(['YearOfBirth', 'Sex'])['pop']
        
        # Fill data for this dose group (same as original)
        ...
        print(f"    Completed dose group {d}")
    ...
    # Add DCCI column (placeholder for now - will be filled with actual DCCI values)
    out['DCCI'] = 0  # Default DCCI value
