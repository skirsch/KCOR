...
\section{KCOR Supporting Information
(SI)}\label{kcor-supporting-information-si}
...
@tbl:appendixD\_assumptions\_diagnostics.
...
@fig:pos\_control\_injected and Table @tbl:pos\_control\_summary confirm
this behavior.
...
The synthetic negative control (Figure @fig:neg\_control\_synthetic) is
generated using:
...
The empirical negative control (Figures @fig:neg\_control\_10yr and
@fig:neg\_control\_20yr) is generated using:
...
The positive control (Figure @fig:pos\_control\_injected and Table
@tbl:pos\_control\_summary) is generated using:
...
The sensitivity analysis (Figure @fig:sensitivity\_overview) varies:
...
\] \{\#eq:appendixB\_individual\_hazard\_with\_effect\}
...
Supporting Information, SI).}\label{fig:neg_control_20yr}
\end{figure}
...
\] \{\#eq:appendixC\_theta\_near\_zero\}
...
Table @tbl:appendixC\_diagnostic\_gate summarizes these diagnostic
...
\[
\hat{\theta}_d > 0.
\] \{\#eq:appendixC\_theta\_positive\}

for Dose 0 cohorts and

\[
\hat{\theta}_d \approx 0.
\] \{\#eq:appendixC\_theta\_near\_zero\_dose2\}
...
@tbl:appendixC\_frailty\_variance demonstrates selection-induced
...
Table @tbl:appendixC\_raw\_hazards for raw cumulative hazards reported
as a pre-normalization diagnostic. KCOR normalization removes the
depletion component, enabling interpretable comparison of the remaining
differences.
...
@tbl:appendixC\_raw\_hazards reports raw outcome contrasts for ages 40+
(YOB \(\le 1980\)) where event counts are stable.
...
Figure @fig:appendixC\_allages shows \(\mathrm{KCOR}(t)\) trajectories
for dose 2 and dose 3 relative to dose 0 for an all-ages analysis. We
