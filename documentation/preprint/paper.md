...
- **Static HVE:** baseline differences in latent frailty distributions at cohort entry (e.g., vaccinated cohorts are healthier on average). In the KCOR framework, this manifests as differing enrollment-time frailty variance $\theta_{0,d}$ and is the primary target of frailty normalization. In practice, this adjustment is not limited to one named bias mechanism; it absorbs any enrollment-time baseline difference that enters the cohort comparison through the same depletion / frailty geometry.
...
> - **Interpretation**: KCOR is a time-indexed **cumulative** contrast on the depletion-neutralized scale. Values above/below 1 indicate greater/less cumulative event accumulation in cohort $A$ than $B$ by time $t$ after depletion normalization. KCOR is not an instantaneous hazard ratio. It should be read as a **diagnostic** signal about persistent contrasts after working-model depletion adjustment, not—by itself—as evidence of causal harm or benefit. In particular, baseline differences that operate through depletion geometry are adjusted out; residual differences should be interpreted as what remains after that depletion-based normalization, not as automatically causal.
...
Bootstrap intervals are calibrated under the working gamma frailty model. An end-to-end simulation pipeline now evaluates empirical bootstrap coverage by forming a percentile interval for each simulated dataset and computing the Monte Carlo fraction of intervals that contain the scenario truth (KCOR at the evaluation week from the data-generating process). **Stable numerical coverage estimates require large numbers of simulated datasets and bootstrap replicates; final coverage claims for the manuscript are deferred until that full validation run is completed.** Estimates are intended to be interpreted conditionally on diagnostic pass status.

The bootstrap is used as a **variance-propagation** device for the aggregated process rather than a universally calibrated inferential procedure. Sub-nominal coverage arises in regimes where model assumptions are violated or curvature is insufficient for identification. In those regimes, KCOR diagnostics explicitly flag non-identifiability and results are not interpreted. Reported percentile intervals are **conditional on passing identifiability diagnostics**. **Whether empirical coverage is close to nominal in well-identified settings is assessed with the Monte Carlo pipeline summarized in Table @tbl:bootstrap_coverage; stable numerical estimates require large $n_{\mathrm{sim}}$ and $n_{\mathrm{boot}}$, and the manuscript does not treat pilot outputs as definitive calibration.** **Inference is therefore interpreted conditional on diagnostic acceptance.**
...
Positive controls in §3.2 inject multiplicative hazard shifts in an **effect** window to test whether KCOR detects a true contrast. A distinct and important concern is whether **non-frailty** additive curvature confined to the **quiet** window itself could bias $\hat\theta_{0,d}$ sufficiently to shift anchored KCOR away from a true null—and in particular whether it could produce sign reversal (KCOR $>1$ when the true contrast is null). We ran a synthetic experiment to bound this empirically.

**Design.** Two cohorts were simulated under the Gompertz-plus-gamma-frailty DGP with known ground-truth parameters: cohort A (lower frailty, $\theta_{0,A}=0.5$) and cohort B (higher frailty, $\theta_{0,B}=4.0$), with shared baseline scale $k$ calibrated to Czech KCOR_CMR weekly hazard for YoB 1940–1960, dose 0 reference ($h_{\mathrm{ref}}=0.000456$ per week). The true cumulative contrast is null by construction (identical baseline hazard). Additive non-frailty contamination was applied **only** to cohort A during rebased quiet-window weeks 4–56. Three contamination types were tested: **Type 1** (decaying dynamic-HVE-style hazard: $g(t)=\epsilon_{\mathrm{dyn}}\exp(-0.05\,t_{\mathrm{rebased}})$); **Type 2** (constant comorbidity-direction hazard: $g(t)=\epsilon_{\mathrm{comorb}}$); and **Type 3** (combined, full $4\times4$ outer product of positive $\epsilon_{\mathrm{dyn}}$ and $\epsilon_{\mathrm{comorb}}$ values, 16 cells). Each cell used 20 stochastic replicates with the production pipeline (`fit_theta0_gompertz`, inversion, anchored KCOR).

**Results.** Figure @fig:quiet_contam_kcor shows mean anchored KCOR asymptote and Figure @fig:quiet_contam_theta shows $\hat\theta_{0,A}$ error and combined sign-reversal frequency.

*Type 1 (dynamic):* Contamination drove $\hat\theta_{0,A}$ to the floor (error $\approx -0.5$, i.e., $\hat\theta_{0,A}\to0$) and KCOR **below** null in the mean as $\epsilon_{\mathrm{dyn}}$ increases—reaching mean KCOR $\approx0.78$ at $\epsilon_{\mathrm{dyn}}=5\times10^{-3}$, with all replicates at or below 1 at that amplitude. For smaller positive $\epsilon_{\mathrm{dyn}}$, a minority of replicates can still exceed 1 even though the **mean** asymptote stays below 1 (Table @tbl:quiet_contam and `quiet_contamination_agg.csv`). Dynamic HVE-style contamination under-corrects cohort A, biasing KCOR toward apparent benefit, not harm.

*Type 2 (comorbidity):* KCOR increased monotonically with $\epsilon_{\mathrm{comorb}}$. Sign reversal (mean KCOR $>1$) first occurred at $\epsilon_{\mathrm{comorb}}\approx3\times10^{-4}$, which equals $66\%$ of $h_{\mathrm{ref}}$—a constant additive weekly hazard two-thirds the size of baseline all-cause mortality for this age stratum. The comorbidity-direction contrasts documented for Czech vaccination cohorts (e.g.\ Šmíd et al., 2024) do not support a hazard increment of that magnitude from measured comorbidity imbalance alone.

*Type 3 (combined):* The joint reversal-frequency heatmap (Figure @fig:quiet_contam_theta, right panel) shows cross-cell variation. At $\epsilon_{\mathrm{comorb}}=3\times10^{-4}$ ($66\%$ of $h_{\mathrm{ref}}$), mean KCOR and reversal fractions can already be elevated for some dynamic levels (e.g.\ $\epsilon_{\mathrm{dyn}}=5\times10^{-4}$: mean KCOR $1.02$, $60\%$ of replicates with $\mathrm{KCOR}_\infty>1$; Table @tbl:quiet_contam). At $\epsilon_{\mathrm{comorb}}=10^{-4}$ ($22\%$ of $h_{\mathrm{ref}}$), reversal fractions stay at or below $40\%$ across the tested $\epsilon_{\mathrm{dyn}}$ grid. Nearly universal reversal within the joint grid requires larger comorbidity-style increments—notably $\epsilon_{\mathrm{comorb}}\ge10^{-3}$ ($219\%$ of $h_{\mathrm{ref}}$) in the illustrative cells in Table @tbl:quiet_contam. Positive dynamic amplitudes on the Type 1 sweep yield mean KCOR below 1 in the aggregated file (with replicate-level exceedances possible at small $\epsilon_{\mathrm{dyn}}$).

**Interpretation.** Substantial apparent harm on the anchored scale requires additive quiet-window perturbations on the order of a large fraction of—or multiples of—baseline weekly mortality, well above plausible magnitudes for the mechanisms often cited as residual confounders. The dynamic-HVE mechanism biases KCOR in the *opposite* direction (toward apparent benefit). Because the dynamic-HVE and comorbidity contamination components act in opposite directions, the net bias direction is an empirical question; neither universal conservatism nor universal overstatement can be assumed without empirically bounding the magnitude of both components independently. These results bound the contamination hypothesis empirically and support the adequacy of the working model for the reported Czech cohorts.

![Mean anchored KCOR asymptote versus contamination magnitude. Left: Type 1 (dynamic HVE style)—mean KCOR falls as $\epsilon_{\mathrm{dyn}}$ increases among positive amplitudes (largest tested: mean $\approx0.78$); small $\epsilon_{\mathrm{dyn}}$ can still leave a minority of replicates above 1. Center: Type 2 (comorbidity style)—KCOR increases; sign reversal threshold at $\epsilon_{\mathrm{comorb}}\approx3\times10^{-4}=66\%$ of $h_{\mathrm{ref}}$. Right: Type 3 combined heatmap—mean KCOR asymptote on the $4\times4$ joint grid. Error bars are $\pm1$ SD across 20 replicates. Baseline hazard scale $h_{\mathrm{ref}}=0.000456$/week (Czech YoB 1940–1960, dose 0).](figures/fig_quiet_contam_kcor_asymptote.png){#fig:quiet_contam_kcor}

![Left and center: mean $\hat\theta_{0,A}$ error versus contamination magnitude for Types 1 and 2. Right: fraction of replicates with anchored $\mathrm{KCOR}_\infty>1$ on the combined $4\times4$ grid (joint contamination).](figures/fig_quiet_contam_theta_error.png){#fig:quiet_contam_theta}

| Contamination type | $\epsilon_{\mathrm{dyn}}$ | $\epsilon_{\mathrm{comorb}}$ | $\epsilon/h_{\mathrm{ref}}$ (comorb) | Mean KCOR$_\infty$ | Frac. reps KCOR$>1$ |
|---|---|---|---|---|---|
...
| Combined | $5\times10^{-4}$ | $3\times10^{-4}$ | 66% | 1.016 | 0.60 |
| Combined | $1\times10^{-3}$ | $3\times10^{-4}$ | 66% | 0.992 | 0.50 |
| Combined | $5\times10^{-4}$ | $1\times10^{-3}$ | 219% | 1.188 | 1.00 |

Table: Selected cells from quiet-window contamination experiment. $h_{\mathrm{ref}}=0.000456$/week (Czech YoB 1940–1960, enrollment sheet `2021_24`). For positive $\epsilon_{\mathrm{dyn}}$ on the Type 1 sweep, mean KCOR stays below 1 in the aggregated runs; comorbidity-only **mean** reversal first appears near $\epsilon_{\mathrm{comorb}}=3\times10^{-4}$. Full grid in `test/quiet_window_contamination/out/quiet_contamination_agg.csv`. {#tbl:quiet_contam}
...
- **Frailty exogeneity assumption**: KCOR assumes that frailty differences between cohorts are pre-treatment in origin, reflecting selection into cohorts rather than biological or behavioral response to the intervention. This assumption is standard in frailty-corrected observational analysis and is not directly testable from mortality data alone. To the extent that frailty is partly endogenous to treatment—for example, through intervention-induced immune modulation or differential post-treatment behavioral change—the estimated frailty ratio would absorb a mixture of selection and causal signal, and KCOR's normalization would be conservative with respect to the true effect. The 2022_06 enrollment cohort (Table @tbl:neg_control_summary) provides partial empirical evidence against large post-treatment frailty shifts, as flat KCOR curves in that period are inconsistent with systematic treatment-induced frailty changes detectable at the population level.
...
Table: Bootstrap coverage for KCOR uncertainty intervals (**pilot / implementation table**). For each independent simulated dataset, a 95% percentile interval for $\mathrm{KCOR}(t)$ at the evaluation week is formed from bootstrap replicates of *that* dataset; empirical coverage is the Monte Carlo fraction of datasets whose interval contains the DGP $\mathrm{KCOR}(t)$ at that week (truth from a fixed reference draw; see `compute_bootstrap_coverage.py` and run manifest `test/sim_grid/out/bootstrap_coverage_run.json`). Summaries are written to `test/sim_grid/out/bootstrap_coverage.csv`; figures may be regenerated with `test/sim_grid/code/plot_bootstrap_coverage.py`. **Empirical coverage cells are left blank until a full run is committed; do not treat small-$n$ smoke values as final.** Script defaults use $n_{\mathrm{sim}}=500$, $n_{\mathrm{boot}}=200$. Stress-test regimes (non-gamma frailty, sparse events) are expected to show sub-nominal coverage at sufficient Monte Carlo replication when diagnostics fail. Coverage is **not** claimed to be universally valid outside diagnostic-pass settings. {#tbl:bootstrap_coverage}

| Scenario | Nominal coverage | Empirical coverage | Notes |
|----------|-----------------|-------------------|-------|
| Gamma-frailty null | 95% | — | Populate after full Monte Carlo run |
| Injected effect (harm) | 95% | — | DGP truth at evaluation week (not raw hazard multiplier) |
| Injected effect (benefit) | 95% | — | DGP truth at evaluation week (not raw hazard multiplier) |
