...
This formulation already normalizes for cohort size at risk. KCOR does not compare raw death counts; all downstream comparisons are performed in hazard space, where population size differences are explicitly accounted for at each event-time interval.
...
Curvature normalization removes cohort-specific intrinsic hazard dynamics while preserving relative event timing. This transformation is applied independently to each cohort and does not introduce cross-cohort adjustment or reweighting. Its sole purpose is to neutralize differences in mortality shape induced by selection.
...
Anchoring fixes the arbitrary multiplicative scale of curvature-normalized hazards by equating cumulative adjusted hazard over a prespecified early post-enrollment window. This step fixes units rather than altering relative mortality and does not affect KCOR values beyond scale normalization. Equivalent results are obtained under symmetric or one-sided scaling.

### 2.9.1 Identifiability and scale fixing after curvature normalization
...
Sensitivity analyses are prespecified and used to assess robustness of the KCOR estimator to reasonable variations in modeling choices. They are not used to optimize or select results.

### 2.12 Uncertainty quantification

Monte Carlo resampling captures uncertainty from death counts and slope estimation.

Uncertainty intervals reflect stochastic event realization and model-fit uncertainty in hazard slope estimation. They do not assume sampling from a superpopulation and are therefore not interpreted as classical confidence intervals for a population mean.
