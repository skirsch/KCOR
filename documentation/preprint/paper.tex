...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Simulated-null summary: KCOR bias as a function of frailty variance \textbackslash theta. Bias is defined as \textbackslash mathrm\{KCOR\}\_\{\textbackslash mathrm\{asymptote\}\} - 1 at the end of follow-up in the synthetic-null grid (no treatment effect). Points show single-run estimates from the grid; no error bars are shown.}]{figures/fig_si_kcor_bias_vs_theta.png}}
...
the synthetic-null grid (no treatment effect). Points show single-run
estimates from the grid; no error bars are
