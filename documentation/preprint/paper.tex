...
40+ (YOB ≤ 1980) where event counts are stable.
...
behavior is not an artifact of age aggregation. See Figures
\ref{fig:appendixC_yob1930}, \ref{fig:appendixC_yob1940}, and
\ref{fig:appendixC_yob1950} for representative examples.
...
depletion dynamics. This appendix uses the same Czech administrative
mortality and vaccination dataset referenced in the empirical negative
control (§3.1.2) and ethics statement.\textsuperscript{12}
...
substantial baseline hazard and frailty variation. To assess whether
apparent stability could arise from cancellation across strata, we also
present narrow birth-year cohorts spanning advanced ages (1930, 1940,
1950) in Figures
\ref{fig:appendixC_yob1930}--\ref{fig:appendixC_yob1950}. Across
aggregation levels, \(\mathrm{KCOR}(t)\) remains stable over the
evaluation window after depletion normalization, consistent with
effective removal of selection-induced curvature in a real-data setting.
These figures are presented as illustrative applications demonstrating
estimator behavior on registry data and do not support causal inference;
...
repository's KCOR defaults are summarized in Table
\ref{tbl:appendixE_defaults}.
