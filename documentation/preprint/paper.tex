...
Diagnostics corresponding to each assumption are summarized in Table
\ref{tbl:appendixD_assumptions_diagnostics} and discussed in detail in
...
\textbf{Failure signals (do not report KCOR as identified).} Treat the
analysis as not identified if any cohort shows: (i) poor fit in
cumulative-hazard space over the quiet window; (ii) persistent
post-normalization nonlinearity within the quiet window; or (iii)
instability of \((\hat{k}_d,\hat{\theta}_d)\) under small boundary
perturbations (e.g., ±4 weeks).
...
\begin{equation}\protect\phantomsection\label{eq:appendixC_residuals}{
...
illustration; additional age-shift variants are provided in the Online
Supplement.
...
Table \ref{tbl:neg_control_summary} provides numeric summaries (Online
Supplement).
...
\textbf{Scope statement (non-causal positioning).} As summarized in Box
1 (§1.6), KCOR is a normalization-and-diagnostic framework rather than a
causal identification strategy. It produces a depletion-neutralized
\textbf{cumulative} contrast whose interpretability is conditional on
explicit assumptions and on internal diagnostics; when diagnostics fail,
KCOR is intended to signal non-identifiability rather than silently
...
\subsubsection{Appendix C tables}\label{appendix-c-tables}
...
\subsubsection{Appendix D tables}\label{appendix-d-tables}
...
\subsubsection{Appendix E tables}\label{appendix-e-tables}
...
\begin{equation}\protect\phantomsection\label{eq:appendixA_individual_hazard}{
h_{i,d}(t) = z_{i,d} \, h_{0,d}(t).
}\end{equation}
...
\begin{equation}\protect\phantomsection\label{eq:appendixA_individual_survival}{
S_{i,d}(t) = \exp\!\left(-z_{i,d} H_{0,d}(t)\right).
}\end{equation}

denote the individual survival function, where

\begin{equation}\protect\phantomsection\label{eq:appendixA_baseline_cumhazard}{
H_{0,d}(t) = \int_0^t h_{0,d}(s) \, ds.
}\end{equation}
...
\begin{equation}\protect\phantomsection\label{eq:appendixA_cohort_survival}{
...
\begin{equation}\protect\phantomsection\label{eq:appendixA_cohort_hazard}{
...
\begin{equation}\protect\phantomsection\label{eq:appendixA_gamma_laplace}{
\mathcal{L}_z(s) = \left(1 + \theta_d s\right)^{-1/\theta_d}.
}\end{equation}

The cohort survival function becomes:

\begin{equation}\protect\phantomsection\label{eq:appendixA_gamma_cohort_survival}{
S^{\mathrm{cohort}}_{d}(t) = \left(1 + \theta_d H_{0,d}(t)\right)^{-1/\theta_d}.
}\end{equation}

The observed cumulative hazard is defined as:

\begin{equation}\protect\phantomsection\label{eq:appendixA_hobs_def}{
...
\begin{equation}\protect\phantomsection\label{eq:appendixA_gamma_identity}{
H_{\mathrm{obs},d}(t)
=
\frac{1}{\theta_d}
...
\begin{equation}\protect\phantomsection\label{eq:appendixA_gamma_inversion}{
H_{0,d}(t)
=
...
\subsubsection{A.3.1 Relationship to the Vaupel--Manton--Stallard gamma
frailty
framework}\label{a.3.1-relationship-to-the-vaupelmantonstallard-gamma-frailty-framework}
...
\begin{equation}\protect\phantomsection\label{eq:appendixA_kcor_def}{
...
\begin{equation}\protect\phantomsection\label{eq:appendixA_kcor_var_delta}{
...
Control-test specifications and simulation parameters are provided in
the Supporting Information (SI), Section S4.
...
Appendix C material (additional figures, diagnostics, and extended Czech
illustrative application) is provided in the Supporting Information
(SI), Sections S5--S6.
...
Appendix D material is provided in the Supporting Information (SI),
Section S2.
...
Appendix E material (reference implementation and default operational
settings) is provided in the Supporting Information (SI), Section S7.
