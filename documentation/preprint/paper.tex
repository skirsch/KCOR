...
\caption{Bootstrap coverage for KCOR uncertainty intervals. For each
independent simulated dataset, a 95\% percentile interval for
$\mathrm{KCOR}(t)$ at the evaluation week is formed from bootstrap
replicates of \emph{that} dataset; empirical coverage is the Monte Carlo
fraction of datasets whose interval contains the true $\mathrm{KCOR}(t)$
at the same week (values in \texttt{test/sim\_grid/out/bootstrap\_coverage.csv};
pilot run shown used $n_{\mathrm{sim}}=2$, $n_{\mathrm{boot}}=12$; defaults in
the script are $500 \times 200$). Nominal 95\% is compared to this empirical
fraction. Stress-test regimes (non-gamma frailty, sparse events) are expected
to show sub-nominal coverage at sufficient Monte Carlo replication when
diagnostics fail, because working-model intervals can be anti-conservative;
coverage is \textbf{not} claimed to be universally valid outside
diagnostic-pass settings.}\label{tbl:bootstrap_coverage}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
Scenario
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\bottomrule\noalign{}
\endlastfoot
Gamma-frailty null & 95\% & 100.0\% & Coverage evaluated under
selection-only conditions \\
Injected effect (harm) & 95\% & 100.0\% & Coverage evaluated under known
treatment effect (true $\mathrm{KCOR}$ at evaluation week from DGP) \\
Injected effect (benefit) & 95\% & 100.0\% & Coverage evaluated under known
treatment effect (true $\mathrm{KCOR}$ at evaluation week from DGP) \\
Non-gamma frailty & 95\% & 100.0\% & Coverage under frailty
misspecification \\
