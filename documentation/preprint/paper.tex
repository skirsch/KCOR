...
Fitting is performed in cumulative-hazard space rather than by
maximizing a likelihood because the primary inputs are discrete-time,
cohort-aggregated hazards rather than individual-level event histories.
Least-squares fitting in cumulative-hazard space serves as a stable
estimating equation for selection-induced depletion geometry during
quiet periods, emphasizes agreement in hazard shape rather than
instantaneous risk, and yields diagnostics (e.g., RMSE and residual
structure in \(H\)-space) that directly assess depletion
identifiability. Likelihood-based fitting can be treated as a
sensitivity analysis but is not required for the gamma-frailty
