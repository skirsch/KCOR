...
gamma-frailty inversion. In epidemic-wave applications, an optional
non-proportional-hazard (NPH) extension rescales wave-period hazards
before cumulative-hazard accumulation and inversion. All normalization
is conditional on the adequacy of the gamma-frailty working model and
Gompertz structure. Across \(\theta_0\) recovery simulations, negative
and positive controls, and Cox comparison exercises, KCOR remains a
...
inversion. That extension is context-specific and is not part of the
...
optional epidemic-wave NPH extension module; and (iv) it validates the
revised estimator using a broader stack that includes \(\theta_0\)
recovery, control analyses, Cox mismatch demonstrations, and explicit
failure signaling.
...
depletion-neutralized cumulative hazard scale. We use a tilde (e.g.,
\(\tilde H_{0,d}(t)\)) to denote depletion-neutralized baseline
...
\(t=0\), not a late-window summary of an already depleted cohort.
Sensitivity to the Gompertz slope parameter \(\gamma\) is evaluated in
the Supplementary Information; results are stable over a prespecified
range, indicating that identification is not driven by a single slope
choice. Identification is driven by curvature structure rather than the
exact functional form of the baseline hazard.
...
§2.7.1. When used, this preprocessing is applied prior to inversion to
preserve the validity of the gamma-frailty mapping between observed and
baseline cumulative hazards. Pooling in this procedure occurs only
across prespecified quiet windows within the same cohort; there is no
shared \(\theta_0\) parameter across cohorts.
...
The NPH extension is not identified from the same data used for frailty
fitting and must be specified independently. In COVID-era applications,
this extension corresponds to the NPH correction described in the v7.5
specification. When the extension is not used,
...
\textbf{(C)} In epidemic-wave applications, an optional NPH extension
may define the preprocessing hazard stream before accumulation and
inversion; this module is specified independently and is not part of the
universal KCOR core. \emph{This schematic is illustrative rather than
empirical. In the schematic, \(\tilde H_{0,d}(t)\) denotes the
depletion-neutralized baseline cumulative hazard and \(\theta_{0,d}\)
denotes enrollment-time frailty variance after the stabilization
...
remains informative as a diagnostic implication under the working model,
but it is no longer the sole validation anchor.
...
\item
  If used, rationale, independent specification, and diagnostics for the
...
\item
  \textbf{Optional NPH module risk}: If the epidemic-wave extension is
  misspecified, it can introduce bias rather than remove it; that module
  must therefore remain externally motivated and diagnostically checked.
\item
...
required for the universal KCOR estimator, it is not identified from the
same data used for frailty fitting, and its validity depends on an
external epidemiologic argument for the chosen rescaling.
...
time-varying forces. Incorrect rescaling can introduce bias rather than
remove it. Analyses spanning major epidemic waves therefore remain more
assumption-sensitive than analyses anchored in quiet periods alone.
...
model-dependent estimates. The resulting contrast remains descriptive
and is interpretable only to the extent that the gamma-frailty working
model, Gompertz structure, and associated diagnostics are adequate. In
epidemic-wave settings, an optional NPH extension may be used, but it
remains an assumption-bearing module rather than a universal part of the
KCOR core.
...
is used, the chosen wave-period hazard rescaling is externally justified
and specified independently of frailty fitting. & Supports wave-period
preprocessing before inversion \\
\end{longtable}
...
the working model rather than assumption-free.
...
aligned quiet-window behavior, and depletion-normalized cumulative
trajectories, but the Czech application remains illustrative rather than
validating. While these examples illustrate KCOR's application, the
method is general and applies to any retrospective cohort comparison
