...
stabilization skip. The core estimator proceeds in four steps. It fits a
seed \((k_d,\theta_{0,d}^{(0)})\) in the nearest quiet window,
reconstructs the effective cumulative hazard over the full trajectory,
aligns multiple quiet windows through persistent wave-offset terms, and
refits \(\theta_{0,d}\) jointly across all prespecified quiet windows
before applying gamma-frailty inversion. In epidemic-wave applications,
an optional non-proportional-hazard (NPH) extension rescales wave-period
hazards before cumulative-hazard accumulation and inversion. All
normalization is conditional on the adequacy of the gamma-frailty
working model and Gompertz structure. Across \(\theta_0\) recovery
simulations, negative and positive controls, and Cox comparison
...
hazard shift within a quiet window without additional information.
...
cumulative hazards. Full notation is summarized in Table
\ref{tbl:notation}.
...
This identity shows how observed cumulative hazard bends away from the
latent baseline under depletion, and the inversion reverses that bend
once \(\theta_{0,d}\) is specified. This inversion is the
\textbf{normalization operator}: given an estimate
...
observed weeks rather than only inside the fit window.
...
\item
  ...
  would be consistent with successful depletion normalization under the
  working model.
\end{itemize}
...
hazard ratios in Cox regression, while KCOR stays close to the null
benchmark under the same conditions.
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={\textbackslash mathrm\{KCOR\}(t) stays near the null benchmark under a synthetic null across increasing frailty heterogeneity. In these simulations, \textbackslash mathrm\{KCOR\}(t) asymptotes remain near 1 across \textbackslash theta\_0, which is consistent with successful depletion normalization under the working model. Uncertainty bands (95\% bootstrap intervals) are shown but are narrow due to large sample sizes.}]{figures/fig_cox_bias_kcor_vs_theta.png}}
...
\(\theta_0\), which is consistent with successful depletion
normalization under the working model. Uncertainty bands (95\% bootstrap
...
the behavior expected when the working model matches the data-generating
process.
...
\item
  ...
  sampling variability and model adequacy.
\item
...
an arbitrary mortality trajectory. Identification depends on a joint
...
consistent with the expected null benchmark under the working model.
(Exact values depend on simulation seed and follow-up
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Synthetic negative control under strong selection (different curvature) but no effect: \textbackslash mathrm\{KCOR\}(t) stays near 1 under the synthetic null. Top panel shows cohort hazards with different frailty-mixture weights inducing different curvature. Bottom panel shows \textbackslash mathrm\{KCOR\}(t) remaining near 1.0 after normalization, which is consistent with successful depletion-neutralization under the working model. Uncertainty bands (95\% bootstrap intervals; aggregated cohort--time resampling) are shown.}]{figures/fig_neg_control_synthetic.png}}
...
consistent with successful depletion-neutralization under the working
model. Uncertainty bands (95\% bootstrap intervals; aggregated
cohort--time resampling) are shown.}\label{fig:neg_control_synthetic}
\end{figure}
