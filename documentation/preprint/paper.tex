...
cross-cohort structure. Identification of depletion geometry is
curvature-based and invariant to constant multiplicative hazard effects
when scaling is common or approximately common across cohorts, while the
optional NPH exponent is not separately identifiable from
cohort-specific multiplicative effects under minimal aggregated data.
Because identification comes from relative cohort behavior rather than
absolute hazard levels, \(\alpha\) is invariant to unknown common wave
amplitudes under the working model. This extension remains optional,
...
\paragraph{2.7.3 NPH estimation procedure
summary}\label{nph-estimation-procedure-summary}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  Construct cohort-week data for fixed cohorts over the target wave
  period.
\item
  Estimate baseline depletion geometry using quiet-window data to obtain
  θ₀ for each cohort.
\item
  Define excess hazard relative to a reference cohort or baseline:
  h\_excess,d(t) = h\_d(t) − h\_ref(t)
\item
  For a grid of α values, compute: F\_d(t; α) = E{[}z\^{}α \textbar{}
  t{]} under the gamma-frailty working model.
\item
  Evaluate two estimators:

  \begin{enumerate}
  \def\labelenumii{(\alph{enumii})}
  \tightlist
  \item
    pairwise cross-cohort log-ratio fit,
  \item
    common-wave collapse estimator.
  \end{enumerate}
\item
  Select α by minimizing the objective function(s), and assess
  ...
  \end{itemize}
\item
  If diagnostics fail, report α as not identified rather than estimated.
\end{enumerate}
...
We evaluated estimation of the NPH exponent \(\alpha\) using a staged
validation framework consisting of synthetic recovery, pooled empirical
estimation, and diagnostic sensitivity analyses. Throughout, \(\alpha\)
is treated as a parameter of the working model describing how excess
hazard scales with latent frailty, rather than as a causal or biological
quantity.
...
on cross-cohort separation in depletion geometry and stable
excess-hazard measurement, and \(\alpha\) is not separately identifiable
from cohort-specific multiplicative effects under minimal data (main
