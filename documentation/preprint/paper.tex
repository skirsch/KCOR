...
geometry and localizes a single shared exponent \(\alpha\) using
cross-cohort structure together with \textbf{externally supplied VE or
other prespecified intervention-effect scale} under minimal aggregated
data. Identification of depletion geometry is curvature-based and
invariant to constant multiplicative hazard effects when scaling is
common or approximately common across cohorts, while \(\alpha\) is not
separately identifiable from cohort-specific multiplicative effects if
that external scale is not fixed. Wave-period NPH adjustment is applied
after inversion only to \textbf{frailty-neutral cumulative hazard}, not
to raw observed hazard (§2.7.3). Because identification comes from
relative cohort behavior rather than absolute hazard levels, \(\alpha\)
is invariant to unknown common wave amplitudes under the working model
once the external scale is specified. This extension remains optional,
diagnostic, and assumption-dependent rather than part of the universal
KCOR core. Across \(\theta_0\) recovery simulations, negative and
positive controls, and Cox comparison exercises, KCOR remains a
diagnostic and descriptive framework: it targets cumulative contrasts
after depletion normalization rather than counterfactual effects. This
revised architecture replaces the earlier quiet-window fit under a
flatter baseline specification with a more explicit identification
...
Scope and interpretation are summarized in Box 2 (§1.6). An informal
statement of conditions under which curvature-based interpretation of
\(\theta_{0,d}\) is intended is given immediately after §2.4.4.
...
\begin{quote}
\textbf{Identification (informal statement).}\\
Curvature-based interpretation of enrollment-time depletion geometry
\(\theta_{0,d}\) under the KCOR working model is intended when: (i)
multiplicative hazard scaling is \textbf{common or approximately common}
across cohorts within the identification regime, so constant rescaling
does not mimic depletion curvature; (ii) \textbf{additive or strongly
time-varying} contamination in quiet windows is limited enough that
residual curvature is not dominated by non-depletion effects; and (iii)
observed cumulative hazards exhibit \textbf{sufficient depletion-linked
curvature} to distinguish \(\theta_{0,d}\) from baseline scale within
prespecified windows after rebasing and offset alignment. A formal
nonparametric identification theorem is \textbf{not} claimed;
identification is \textbf{conditional} on the gamma-frailty and Gompertz
working model and is evaluated \textbf{diagnostically} (§2.1.2; SI §S2).
\end{quote}
